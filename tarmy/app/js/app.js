(function ($){
    $(function () {
        ...
        var apiURL = window.location.href.indexOf("localhost") !== -1 ? "http://localhost:3000/ritwik.app/api" : "http://54.153.14.234:8080/ritwik.app/api";
        ...
        $.get(apiURL + "/feeds/spreadsheets/private/full", {}, function (allsheets) {
            var worksheetFeedURL = $("entry:icontains('tarmy') link[rel='http://schemas.google.com/spreadsheets/2006#worksheetsfeed']", allsheets).attr("href");
            $.get(apiURL + worksheetFeedURL.substring(GOOG_PREFIX.length), {}, function (worksheets) {
                ...
                $.get(apiURL + leadsListFeedUrl.substring(GOOG_PREFIX.length), {}, function (listFeed) {
                    ...
                    $("entry", listFeed).each(function () {
                        ...
                        $([ "leadname", "email", "phonenumber"]).each(function () {
        ...
        var login = function (email, password) {
            ...
            var leadEmailAddresses = [
                "srinivastsv@gmail.com",
                "gchandar@gmail.com",
                "bala_muthu1957@yahoo.com",
                "emailaddepalli@gmail.com",
                "gunashekharm@gmail.com",
                "vijaya.krish@gmail.com",
                "kalyanssk@gmail.com",
                "nskop@hotmail.com",
                "csntrr@hotmail.com",
                "pavan.jella@gmail.com",
                "sboddupalli@gmail.com",
                "pyellai@gmail.com",
            ...
            var $yearOfBirth = $emailCell.closest("entry").filterNode("gsx:dateofbirth");
            var dob = $emailCell.closest("entry").filterNode("gsx:dateofbirth").text();
            var yob = dob.substring(dob.length - 4);  //.filter(":ieq('" + password + "')");
            ...
            //Find the lead in ritwik sheet

            var grouping = $ritwik.filterNode("gsx:lead").text();
            //var $lead = $(Store.worksheets.leads).filterNode("gsx:area").filter(":ieq('" + grouping + "')");
            var $lead = $(Store.worksheets.leads).filterNode("gsx:leadname").filter(":ieq('" + grouping + "')").closest ("entry");
            if ($lead.length > 1) {
        ...
        window.initHomePage = function () {
            ...
            if (ritwikEmail.toLowerCase() === "sboddupalli@gmail.com"
                ...
                || ritwikEmail.toLowerCase() === "srinivastsv@gmail.com") {
            ...
            $table.on("click", "tr", function () {
                ...
                var homakundam = $(this).find("td").eq(4).text();
                var roomcode = $(this).find("td").eq(5).text();
                ...
                $ritwik = $(Store.worksheets.ritwiks)
                    ...
                    .filterNode("gsx:status").filter(":eq('" + status + "')").closest("entry")
                    .filterNode("gsx:homakundam").filter(":eq('" + homakundam + "')").closest("entry")
                    .filterNode("gsx:roomcode").filter(":eq('" + roomcode + "')").closest("entry")
        ...
        var initAccomodationOptions = function () {
            ...
            var hotelName, hotelRoomCode, homaKundaName, homaKundaSpot, homaKundaNumber, hotelRoomRate;

            if ($ritwik.filterNode("gsx:hotelname").text ().trim ()) {
                hotelName = $ritwik.filterNode("gsx:hotelname").text ();
                hotelRoomCode = $ritwik.filterNode("gsx:roomcode").text ();
                homaKundaName = $ritwik.filterNode("gsx:homakundaname").text ();
                homaKundaNumber = $ritwik.filterNode("gsx:homakundanumber").text ();
                homaKundaSpot = $ritwik.filterNode("gsx:homakundam").text ();
                hotelRoomRate = $ritwik.filterNode("gsx:roomrate").text ();
            } else {
                var spouseName = $ritwik.filterNode("gsx:ifspouseisjoiningspousefirstname").text();
                if (spouseName) {
                    var $spouse = $(Store.worksheets.ritwiks).filterNode("gsx:ifspouseisjoiningspousefirstname").filter(":ieq('" + spouseName + "')").closest("entry");
                    if ($spouse.length) {
                        hotelName = $spouse.filterNode("gsx:hotelname").text ();
                        hotelRoomCode = $spouse.filterNode("gsx:roomcode").text ();
                        homaKundaName = $spouse.filterNode("gsx:homakundaname").text ();
                        homaKundaNumber = $spouse.filterNode("gsx:homakundanumber").text ();
                        homaKundaSpot = $spouse.filterNode("gsx:homakundam").text ();
            ...
            $("#hotelRoomNumber").text(hotelRoomCode);
